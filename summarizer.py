...
def categorize_article(title, content):
    ...
    key = hashlib.md5((title + content[:500]).encode()).hexdigest()
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if attempt < max_attempts:
                ...
                print("Maximum retry attempts reached. Unable to categorize.")
                return fallback_categorization(title, content)


def fallback_categorization(title, content):
    keywords = {
...
def index():
    ...
    categories = ['All', 'Sports', 'Entertainment', 'Politics', 'International', 'Others']
    return render_template('index.html', data=data, formatDateTime=format_datetime, categories=categories,
...
def search():
    ...
    try:
        conn = sqlite3.connect('summarizer-data.db')
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM rss_feed WHERE title LIKE ? OR full_content LIKE ?",
                       ('%' + query + '%', '%' + query + '%'))
        data = cursor.fetchall()
...
def filter_by_date():
    ...
    if request.method == 'POST' and date:
        data = get_data(sort_order, category, date)
    else:
        data = []
        date = None

    categories = ['All', 'Sports', 'Entertainment', 'Politics', 'International', 'Others']
    return render_template('filter_by_date.html', data=data, formatDateTime=format_datetime,
                           categories=categories, selected_category=category,
                           selected_date=date, selected_sort_order=sort_order)
